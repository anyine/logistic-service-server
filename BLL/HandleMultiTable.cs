...
namespace BLL
{
    public class HandleMultiTable
    {
        ...
        public static bool HandleLaunchExchangeTable(string poster, string exchanger, int sum, double userstore)
        {
            ...
            SQLStrList.Add(strSql);
            strSql = @"update dbo.plant_user_income  set user_store= user_store-{0} where user_name= '{1}' ";
        ...
        // 交易取消更新相关的表
        public static bool UpdateHandleCancleMyExchange(string id, string username, double userstore)
        {
            bool bRet = false;
            List<String> SQLStrList = new List<String>();
...
//             SQLStrList.Add(strSql);
        ...
        public static int GetTotalPoint(string id)
        {
            ...
            if (dt.Rows.Count < 1)
                return -1;
            return Convert.ToInt32(dt.Rows[0]["user_points"]);
        }
